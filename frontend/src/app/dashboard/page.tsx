...
import {
  getEvents, getInsights, deleteSession,
  type EventRecord,
} from "@/lib/api";
    ...
    <div className="card card-hover" style={{ padding: "18px 20px", borderTop: `3px solid ${color}` }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "8px" }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ margin: 0, fontSize: "11px", fontWeight: 600, color: "#6B7280", textTransform: "uppercase", letterSpacing: "0.06em" }}>{label}</p>
          <p className="font-mono-data" style={{ margin: "5px 0 0", fontSize: "26px", fontWeight: 700, color: "#111827", lineHeight: 1, wordBreak: "break-all" }}>{value}</p>
          {sub && <p style={{ margin: "5px 0 0", fontSize: "11.5px", color: "#6B7280" }}>{sub}</p>}
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetchData();
    polling.current = setInterval(fetchData, 3000);
    ...
    <div className="animate-fade-in" style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <div className={`card${isWarning ? " animate-pulse-warning" : ""}`} style={{ padding: "18px 22px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px", flexWrap: "wrap", gap: "8px" }}>
          <div>
            <p style={{ margin: 0, fontSize: "12px", fontWeight: 600, color: "#6B7280", textTransform: "uppercase", letterSpacing: "0.06em" }}>Budget Consumption</p>
            <p style={{ margin: "3px 0 0", fontSize: "13px", color: "#111827" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: "5px" }}>
          <span className="font-mono-data" style={{ fontSize: "10.5px", color: "#6B7280" }}>$0.00</span>
          <span className="font-mono-data" style={{ fontSize: "10.5px", color: "#6B7280" }}>${budget.max.toFixed(2)}</span>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "16px" }}>
        ...
        <div className="card" style={{ padding: "20px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
            <div>
              <p style={{ margin: 0, fontWeight: 600, color: "#111827", fontSize: "13.5px" }}>Cost Curve</p>
              <p style={{ margin: "2px 0 0", fontSize: "11.5px", color: "#6B7280" }}>Per-query cost + cumulative</p>
            </div>
            <span className="font-mono-data" style={{ fontSize: "10.5px", color: "#6B7280", background: "#F7F9F8", padding: "3px 8px", borderRadius: "5px", border: "1px solid #E3E8E6" }}>{chartData.length} pts</span>
          </div>
          <div style={{ height: "200px" }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 5, right: 5, left: -24, bottom: 0 }}>
                ...
                <XAxis dataKey="time" stroke="#6B7280" tick={{ fontSize: 9 }} axisLine={false} tickLine={false} />
                <YAxis stroke="#6B7280" tick={{ fontSize: 9 }} tickFormatter={v => `$${v.toFixed(3)}`} axisLine={false} tickLine={false} />
                <Tooltip contentStyle={{ background: "#fff", border: "1px solid #E3E8E6", borderRadius: "8px", fontSize: "11px" }} />
                <Area type="monotone" dataKey="cost" name="Per-query" stroke="#0D9488" strokeWidth={2} fill="url(#gTeal)" dot={false} />
                <Area type="monotone" dataKey="cumulative" name="Cumulative" stroke="#6366F1" strokeWidth={1.5} strokeDasharray="5 4" fill="url(#gIndigo)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        {/* Category mix */}
        <div className="card" style={{ padding: "20px" }}>
          <p style={{ margin: "0 0 4px", fontWeight: 600, color: "#111827", fontSize: "13.5px" }}>Category Mix</p>
          <p style={{ margin: "0 0 14px", fontSize: "11.5px", color: "#6B7280" }}>Query distribution by type</p>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginTop: "10px" }}>
                {categoryData.map(d => (
                  <div key={d.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <span className="font-mono-data" style={{ fontSize: "12px", color: "#111827", fontWeight: 600 }}>{d.value}</span>
                  </div>
                ))}
              </div>
            </>
      ...
      <div className="card" style={{ overflow: "hidden" }}>
        <div style={{ padding: "16px 20px", borderBottom: "1px solid #E3E8E6", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px" }}>
          <div>
            <p style={{ margin: 0, fontWeight: 600, color: "#111827", fontSize: "13.5px" }}>Recent Events</p>
            <p style={{ margin: "2px 0 0", fontSize: "11.5px", color: "#6B7280" }}>Latest 8 audit decisions</p>
          </div>
            ...
            <tbody>
              <AnimatePresence initial={false}>
                {events.slice(0, 8).map((r, i) => (
                  ...
                  >
                    <td className="font-mono-data" style={{ color: "#6B7280", fontSize: "11.5px" }}>{formatTime(r.timestamp_ms)}</td>
                    <td>
                      <span style={{ display: "inline-flex", alignItems: "center", gap: "5px", fontSize: "12.5px", fontWeight: 500, color: CATEGORY_COLORS[r.category] ?? "#6B7280" }}>
                        <span style={{ width: 6, height: 6, borderRadius: "50%", background: CATEGORY_COLORS[r.category] ?? "#6B7280", display: "inline-block" }} />
                        {CATEGORY_LABELS[r.category] ?? r.category}
                      </span>
                    </td>
                    <td className="font-mono-data" style={{ fontSize: "11.5px", color: "#374151" }}>{r.audit_event.model ?? "—"}</td>
                    <td className="font-mono-data right" style={{ fontSize: "12px", fontWeight: 600 }}>${r.audit_event.cost_total?.toFixed(5)}</td>
                    <td className="font-mono-data right" style={{ color: "#6B7280", fontSize: "11.5px" }}>{formatLatency(r.audit_event.latency_used_ms ?? 0)}</td>
              ...
              {events.length === 0 && (
                <tr><td colSpan={6}>
                  <div className="empty-state">
                    <p style={{ margin: 0, fontSize: "13px", color: "#6B7280" }}>No events — <Link href="/dashboard/query" style={{ color: "#0D9488", fontWeight: 600 }}>submit a query</Link> to begin.</p>
                  </div>
                </td></tr>
              )}
            </tbody>
        ...
        {[
          ...
          <Link key={item.label} href={item.href} style={{ textDecoration: "none" }}>
            <div className="card card-hover" style={{ padding: "14px 16px", borderLeft: `3px solid ${item.color}`, cursor: "pointer" }}>
              <p style={{ margin: "0 0 3px", fontWeight: 600, color: "#111827", fontSize: "13px" }}>{item.label}</p>
